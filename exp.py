...
def k_fold_cross_validation(config):
    ...
    for fold, (train_idx, test_idx) in enumerate(kfold.split(config.dataset)):
        ...
        test_subset = data.Subset(config.dataset, test_idx)
        ...
        test_loader = data.DataLoader(test_subset, batch_size=config.batch_size, shuffle=False)
...
def eval_concept(config):
    ...
    for it in range(config.num_its):
        ...
        overall_constraint_satisfaction, overall_loss = k_fold_cross_validation(config)
        overall_constraint_satisfactions.append(overall_constraint_satisfaction)
        overall_losses.append(overall_loss)
        
    # Cleanup before re-instantiation
